...
/obj/item/device/megaphone/attack_self(mob/living/carbon/human/user as mob)
	...
	if ((src.loc == user && user.stat == 0))
		if(emagged)
			if(insults)
				user.audible_message("<B>[user]</B> broadcasts, <FONT size=3>\"[pick(insultmsg)]\"</FONT>", null, 10) // 2 stands for hearable message
				insults--

		else
			user.audible_message("<B>[user]</B> broadcasts, <FONT size=5>\"[message]\"</FONT>", null, 10) // 2 stands for hearable message

		playsound(loc, 'sound/items/megaphone.ogg', 100, 0, 1)
		spamcheck = world.time + 50
		return

/obj/item/device/megaphone/attackby(obj/item/weapon/W as obj, mob/user as mob)
