...
/datum/dna/proc/generate_uni_identity(mob/living/carbon/character)
	...
	if(!L[DNA_MUTANTRACE_BLOCK])
		if(prob(60))
			L[DNA_MUTANTRACE_BLOCK] = construct_block(rand(1,14), species_list.len+1)
		else
			L[DNA_MUTANTRACE_BLOCK] = construct_block(1, species_list.len+1)
...
/proc/updateappearance(mob/living/carbon/C)
	...
	if(istype(C, /mob/living/carbon/human))
		...
		if(getblock(structure, DNA_EYE_COLOR_SWITCH_BLOCK)==2)
			H.heterochromia = sanitize_hexcolor(getblock(structure, DNA_EYE_COLOR_TWO_BLOCK))
		else
			H.heterochromia = 0
