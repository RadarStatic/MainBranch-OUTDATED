		...
		if(1)	//nuke was nearby but (mostly) missed
			...
			switch( override )
				if("nuclear emergency") //Nuke wasn't on station when it blew up
					if(cinematic)flick("intro_nuke",cinematic)
					sleep(35)
					world << sound('sound/effects/explosionfar.ogg')
					if(cinematic)flick("station_intact_fade_red",cinematic)
					if(cinematic)cinematic.icon_state = "summary_nukefail"
				if("fake") //The round isn't over, we're just freaking people out for fun
					if(cinematic)flick("intro_nuke",cinematic)
					sleep(20)
					world << sound('sound/items/bikehorn.ogg')
					if(cinematic)flick("summary_selfdes",cinematic)
				else
					if(cinematic)flick("intro_nuke",cinematic)
					sleep(35)
					world << sound('sound/effects/explosionfar.ogg')
		...
		if(2)	//nuke was nowhere nearby	//TODO: a really distant explosion animation
			...
			switch( override )
				if("nuclear emergency") //Nuke Ops successfully bombed the station
					if(cinematic)flick("intro_nuke",cinematic)
					sleep(35)
					if(cinematic)flick("station_explode_fade_red",cinematic)
					world << sound('sound/effects/explosionfar.ogg')
					if(cinematic)cinematic.icon_state = "summary_nukewin"
				if("AI malfunction") //Malf (screen,explosion,summary)
					if(cinematic)flick("intro_malf",cinematic)
					sleep(76)
					flick("station_explode_fade_red",cinematic)
					world << sound('sound/effects/explosionfar.ogg')
					if(cinematic)cinematic.icon_state = "summary_malf"
				if("blob") //Station nuked (nuke,explosion,summary)
					if(cinematic)flick("intro_nuke",cinematic)
					sleep(35)
					if(cinematic)flick("station_explode_fade_red",cinematic)
					world << sound('sound/effects/explosionfar.ogg')
					if(cinematic)cinematic.icon_state = "summary_selfdes"
