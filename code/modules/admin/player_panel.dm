			...
			dat += {"
				...
				<tr id='data[i]' name='[i]' onClick="addToLocked('item[i]','data[i]','notice_span[i]')">
					<td align='center' bgcolor='[color]'>
						...
						<a id='link[i]'
						onmouseover='expand("item[i]","[M_job]","[M_name]","[M_rname]","--unused--","[M_key]","[M.lastKnownIP]",[check_rights(R_SPECIAL,0) ? is_antagonist : null],"\ref[M]")'
						>
...
/datum/admins/proc/player_panel_old()
	...
	for(var/mob/M in mobs)
		...
		"}
		var/show_trait_stat=is_special_character(M)
		if(!check_rights(R_SPECIAL,0))
			show_trait_stat=0
		switch(show_trait_stat)
			if(0)
