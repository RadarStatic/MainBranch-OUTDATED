...
/datum/species
	...
	var/roundstart = 1	// can this mob be chosen at roundstart? (assuming the config option is checked?)
...
/datum/species/proc/handle_chemicals(var/datum/reagent/chem, var/mob/living/carbon/human/H)
	if(chem.id == "mutationtoxin"&&restricted!=2)
...
/datum/species/proc/spec_attack_hand(var/mob/living/carbon/human/M, var/mob/living/carbon/human/H)
	...
	switch(M.a_intent)
		...
		if("harm")
			...
			if(H.stat==DEAD && damage>7 && affecting.name=="chest")
...
/datum/species/proc/handle_breath(datum/gas_mixture/breath, var/mob/living/carbon/human/H)
	...
	if(!breath || (breath.total_moles() == 0) || H.suiciding)
		...
		if(H.suiciding)
			...
			return 0
		if(H.health >= config.health_threshold_crit)
		//NORMAL DAMAGE BLOCK START
			if(NOBREATH in specflags)	return 1
			if(VD)
				if(VD.owner.stat==2||!VD.oxygen)
					H.adjustOxyLoss(HUMAN_CRIT_MAX_OXYLOSS*2)
					H.oxygen_alert = max(H.oxygen_alert, 1)
				H.failed_last_breath=1
				return
			else
				...
				H.failed_last_breath = 1
		//NORMAL DAMAGE BLOCK END
		else
		//CRITICAL DAMAGE BLOCK START
			if(VD&&VD.owner.stat!=2&&VD.oxygen>1)
				H.failed_last_breath=1
				return
			else
				H.adjustOxyLoss(HUMAN_CRIT_MAX_OXYLOSS)
				H.failed_last_breath = 1
		//CRITICAL DAMAGE BLOCK END
