...
//BubbleWrap - Should stop you pushing a restrained person out of the way
		...
		if((tmob.a_intent == "help" || tmob.restrained()) && (a_intent == "help" || src.restrained()) && tmob.canmove && canmove) // mutual brohugs all around!
			...
			var/changeloc=1
			loc = tmob.loc
			if(istype(tmob,/mob/living))
				var/mob/living/L=tmob
				if(sizeplay_size>L.sizeplay_size+1||L.sizeplay_size>sizeplay_size+1) //Step over or run between legs
					changeloc=0
			if(changeloc)
				tmob.loc = oldloc
			now_pushing = 0
		...
		if(a_intent=="harm"&&canmove)//Check for macro stompage!
			if(istype(tmob,/mob/living))
				...
				if(sizeplay_size>L.sizeplay_size+1)
					visible_message("<span class='warning'>[src] steps on [tmob]!</span>")
					loc=tmob.loc
					tmob.Stun(rand(2,4))//Make this falling later
		...
		if(istype(tmob,/mob/living))
			...
			if(L.sizeplay_size>sizeplay_size+1)
				if ( !(world.time % 5) )
					src << "\red [tmob] is too big, you cannot push past!"
				now_pushing = 0
				return
