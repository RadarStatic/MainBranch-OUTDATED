	...
	if (reagents.reagent_list.len > 0)
		switch(reagents.get_master_reagent_id())
			...
			if("semen")
				icon_state = "glass_white"
				name = "Glass of \"milk\""
				var/list/data=reagents.get_reagent_data("semen")
				if(data&&data["digested"]!=null)
					name = "\"Milk\" of [data["digested"]]"
				else if(data&&data["type"])
					if(data["type"]!="human")
						name="Glass of [data["type"]] \"milk\""
				desc = "White and viscous goodness!"
			if("femjuice")
				icon_state = "glass_clear"
				name = "Glass of femjuice"
				var/list/data=reagents.get_reagent_data("femjuice")
				if(data&&data["digested"]!=null)
					name = "[data["digested"]] juice"
				desc = "It's really just urine."
			if("narkychem")
				icon_state = "narkydelightglass"
				name = "Narky Delight"
				var/list/data=reagents.get_reagent_data("narkychem")
				if(data&&data["adjective"])
					if(data["adjective"]!="ordinary")
						name = capitalize(data["adjective"])+" [name]"
				if(data&&data["digested_type"])
					name = name+" (with [data["digested_type"]])"
				desc = "A prettiful drink."
			if("hornychem")
				icon_state  = "love_potion"
				name = "love potion"
				desc = "For when you love me long time."
			if("cream")
				icon_state  = "glass_white"
				name = "glass of cream"
				desc = "Ewwww..."
			...
			if("fuel")
				...
				desc = "It's a glass with [reagents.get_master_reagent_name()] in it."
	else
		icon_state = "glass_empty"
		name = "Drinking glass"
		desc = "Your standard drinking glass"
