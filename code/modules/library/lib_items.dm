...
/obj/structure/bookcase/initialize()
	...
	//NARKY'S BOOKSHELF CODE!
	if(category)
		establish_db_connection()
		if(dbcon.IsConnected())
			var/DBQuery/query = dbcon.NewQuery("SELECT * FROM [format_table_name("library")] WHERE category='[category]' AND isnull(deleted) ORDER BY RAND() LIMIT 5")
			query.Execute()
			...
			while(query.NextRow())
				...
				for(var/obj/machinery/librarycomp/LC in area_contents(get_area(src)))
					LC.inventory+=B
					break
	//END!
