...
/datum/preferences/New(client/C)
	...
	if(istype(C))
		...
		if(!IsGuestKey(C.key))
			...
			if(C.ckey=="kingpygmy") //I need mah extra save slots! ...Then again, maybe I should have given myself a different max_save_slots
				unlock_content = 1
			if(unlock_content)
				max_save_slots = 10
...
/datum/preferences
	proc/ShowChoices(mob/user)
		...
		switch(current_tab)
			if (0) // Character Settings#
				...
				if(eye_het)
					dat += "<span style='border: 1px solid #161616; background-color: #[eye_het];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=eyes_het;task=input'>Heterochromia</a><BR>"
				else
					dat += "<a href='?_src_=prefs;preference=eyes_het;task=input'>Heterochromia?</a><BR>"
			...
			if (1) // Game Preferences
				...
				if(jobban_isbanned(user, "Syndicate"))
					dat += "<b>You are restricted from antagonist roles.</b>"
					src.be_special = 0
				else
					var/n = 0
					for (var/i in special_roles)
						if(special_roles[i]) //if mode is available on the server
							if(jobban_isbanned(user, i))
								dat += "<b>Be [i]:</b> <font color=red><b>\[RESTRICTED]</b></font><br>"
	...
	proc/SetChoices(mob/user, limit = 17, list/splitJobs = list("Chief Engineer"), widthPerColumn = 295, height = 620)
		...
		for(var/datum/job/job in job_master.occupations)
			...
			if(jobban_isbanned(user, rank))
				HTML += "<font color=red>[rank]</font></td><td><font color=red><b> \[RESTRICTED\]</b></font></td></tr>"
				continue
		...
		switch(href_list["task"])
			...
			if("input")
				switch(href_list["preference"])
					...
					if("eyes_het")
						switch(alert("Have heterochromia?","Character Preference","Yes","No","Cancel"))
							if("Yes")
								var/new_color = input(user, "Choose second eye colour:", "Character Preference") as null|color
								if(new_color)
									eye_het = sanitize_hexcolor(new_color)
							if("No")
								eye_het=null
	...
	proc/copy_to(mob/living/carbon/human/character)
		...
		if(eye_het)
			character.heterochromia = eye_het
