	...
	if(ishuman(M) && isliving(user))
		if(user.a_intent == "help")
			...
			if(body_part)
				...
				if(M.stat == DEAD || (M.status_flags&FAKEDEATH))
					...
					switch(body_part)
						...
						if("eyes","mouth")
							...
							sound = "anything"
						if("head")
							if(M.real_name=="Mario Moon"||M.real_name=="Alivia Hunt") //All in good fun.
								sound_strength = "cannot hear"
								sound = "anything"
							if(M.dna&&M.dna.mutantrace()=="narky")
								sound_strength="hear [pick("a weird","a strange","an abnormal")]"
								sound = pick("chain of nyars","rambling about paws","nyaring",
								"string of questions involving how the station only needs floors and walls, but no ceiling",
								"...you know what you hear? You hear stories of pineapples. They're in the Carribean. You decide you want to eat them all")
						else
							sound_strength = "hear a weak"
				...
				//for(var/mob/living/L in M.stomach_contents)
				//	check_peeps=1
				//	break
